...
_<dfn id = "term-battle-item">Battle items</dfn>_ are a variety of inventory item that is usable during [combat](/combat).  Some battle items reproduce the effects of an ability such as Heal or Restore Mana, while others have more unique effects.  Each individual item, once used, is expended and disappears from the [party](/extras/key-terms#term-party) inventory.  All battle items cost 1 [AP](/combat/round#term-ap) to use unless otherwise specified.
...
|Debuff|9 cr|When used, this item inflicts a [debuff](/combat/buffs-and-debuffs#term-debuff) of the specified type.  If it is Mundane, it debuffs by one level on the [Bonus Tracker](/combat/buffs-and-debuffs#term-bonus-tracker), if it is Rare it debuffs by two, and if it is Expert it debuffs by three.|
|Buff|9 cr|When used, this item applies a [buff](/combat/buffs-and-debuffs#term-buff) of the specified type.  If it is Mundane, it buffs by one level on the Bonus Tracker, if it is Rare it buffs by two, and if it is Expert it buffs by three.|
|Inflict Status|12 cr|When you use this item, make a [status-inflicting ability](/combat/attacking-and-defending#term-status-inflicting-ability) roll against the target's [Resistance](/characters/attributes-and-statistics#term-resistance) to determine whether or not the specified [status](/combat/statuses) is inflicted.  If the item is Mundane, do not subtract anything from the roll.  If the item is Rare, subtract 2 from the roll, and if the Expert then subtract 5 from the roll.|
|Revive|15 cr|When you use this item, the targeted character is revived from being [downed](/combat/death-and-injury#term-downed).  If the item is Mundane, they are revived to 1 HP.  If the item is Rare they are revived to half their maximum HP, rounded down, and if the item is Expert then they are revived to their current maximum HP.|
...
|Negate Resistances|50 cr|This item negates all of a target's elemental resistances until the beginning of the target's next [turn](/combat/round#term-turn).|
...
|Summon Creature|1000x(Tier of summoned creature)|When you use this item, it summons a creature, usually an [enemy](/enemies) of some type, to fight as an ally on your side.  The first time it can take its turn is during the [round](/combat/round#term-round) after you summoned it, but it can be targeted as soon as it is summoned.|
...
|Row|This item affects all characters on a single [row](/combat/starting#term-row).  When you use the item, you can choose which row it affects.  Multiplies base cost by 5.|
...
|Delay|When you use this item, it does not trigger immediately.  Instead, its effect happens during the [Effect Phase](/combat/round#term-effect-phase) of the round.  Multiplies cost by 0.75, rounded down.|
