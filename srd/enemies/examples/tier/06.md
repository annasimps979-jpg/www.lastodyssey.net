...
|Screen|Ally|1 AP, 9 MP|Confers the Protected status on the Golem or one of its allies.  The next attack of the Golem's elemental type that hits a [character](/srd/extras/key-terms#term-character) that is Protected will have its damage negated.|
...
|Foretell Doom|Passive|N/A|After the Effect Phase in [battle](/srd/extras/key-terms#term-battle) passes three times, the Prophet makes a magical attack against all of its enemies that does not cost any MP.  Every hit enemy takes magical water damage that is multiplied by 3 in addition to other modifiers.|
