...
|Slime|Ranged|1 AP, 12 MP|Inflicts the Slowed status on a single enemy.  Works the same as the [Saboteur's](/srd/heroes/roles/saboteur) Slow ability.|
...
|Expose Weakness|Ranged|1 AP, 8 MP|Uses a [status-inflicting ability](/srd/combat/attacking-and-defending#term-status-inflicting-ability) that, on a hit, inflicts the Vulnerable (Water) status.  This effect works the same as the Saboteur's Vulnerability power.|
...
|Double Time|Multi|1 AP, 25 MP|Hastes the Chieftain and all of their allies.  Works the same as the Support role's Multihaste ability.|
